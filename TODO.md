...
- [ ] **Verify cost drop after a week of data** - rerun `scripts/analyze_costs.py` on oddjob (~Jun 19); cached share of input should be substantial, daily cost trending down, and cost-weighted `tool_rounds`/turn falling as the model adopts batched/parallel search. If cache_read stays 0, debug the cached_content path.
